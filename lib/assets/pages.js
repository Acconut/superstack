(function(window) {
    ...
    window.Route = function Route(path) {
        ...
        this._target = null;             // Where to inject the template
    ...
    /**
     * Where to render templates default in (default: document.body)
     */
    Pages._target = null;
    ...
    /**
     * Last template to be rendered
     * Used to remove it from the class list
     */
    Pages._lastTemplate = "";
    /**
    ...
    Pages.render = function(template, params, target) {
        ...
        // Change classes
        document.body.className = document.body.className.replace(this._lastTemplate, " " + template + " ");
        
        var tg = target || this._target || document.body;
     ...
     * @param: Status code, only available on server-side (default: 302)
     */
    Pages.redirect = function(url, code) {
        location = url;
    };
